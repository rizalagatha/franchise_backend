...
const verifyToken = (req, res, next) => {
  ...
  if (!secret) {
    return res
      .status(500)
      .json({ message: "Kesalahan server: JWT_SECRET tidak diatur." });
  }

  jwt.verify(token, secret, (err, user) => {
    if (err) {
      // Jika token expired, kirim status 401
      if (err.name === "TokenExpiredError") {
        return res.status(401).json({ message: "Token telah kedaluwarsa." });
      }
      // Jika token tidak valid karena alasan lain
...
// Middleware untuk cek 'insert' ATAU 'edit'
...
// Middleware "pintar" untuk Cek Simpan (Insert/Update)
const checkSavePermission = (menuId) => {
  return async (req, res, next) => {
    // Tentukan action berdasarkan body
    // 'isNew' harus dikirim dari frontend saat menyimpan
